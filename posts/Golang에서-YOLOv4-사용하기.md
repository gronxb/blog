...
백엔드에서 영상을 실시간으로 읽어 객체 탐지한 데이터를 필요로 한다면 처음으로 생각한건 Node.js에서 **opencv4nodejs**  모듈을 사용하는 것이었습니다. 하지만 Node.js 환경에서 진행한다면 **"과연 실시간으로 처리할 퍼포먼스가 나올까?"** 의문이 들었습니다. Golang은 바이너리로 컴파일 되기 때문에 퍼포먼스면으로 얻을 수 있는게 많습니다. 더 나아가 동시성을 지원하기 때문에 HTTP서버를 구동하면서 영상 처리를 할 수 있는 언어라고 생각이 들었습니다. 
